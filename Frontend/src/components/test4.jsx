...
const fetchData = () => {
    ...
    const endpoint = `/run-greedy-${selectedAlgorithm.toLowerCase()}`;
...
const handleStart = async () => {
  ...
  await fetchData(); // sends POST to backend and opens SSE stream

  // Playback starts when steps begin arriving via stream
};

// Call this when SSE receives a new step
const handleNewStepFromBackend = (step) => {
  setSteps(prev => {
    const updated = [...prev, step];

    // Start playing as soon as we receive the first step
    if (updated.length === 1) {
      setIsPlaying(true);
      setCurrentStep(0);
    }
