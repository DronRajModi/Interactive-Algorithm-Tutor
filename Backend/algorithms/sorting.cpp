...
// For explicit split in Merge Sort
void logSplit(const vi &arr, int l, int m, int r) {
    cout << "{";
    cout << "\"type\":\"split\",\"l\":" << l << ",\"m\":" << m << ",\"r\":" << r;
    cout << ",\"left\":[";
    for (int i = l; i <= m; ++i)
        cout << arr[i] << (i < m ? "," : "");
    cout << "],\"right\":[";
...
void merge(vi& arr, int l, int m, int r) {
    logStep(arr, "merge_start", l, r, "Start merging");
    ...
    while (i < n1 && j < n2) {
        ...
        logStep(arr, "array-update", -1, -1, "Merging step");
    }

    while (i < n1) {
        arr[k++] = L[i++];
        logStep(arr, "array-update", -1, -1, "Merging left leftovers");
    }
    while (j < n2) {
        arr[k++] = R[j++];
        logStep(arr, "array-update", -1, -1, "Merging right leftovers");
    }

    logStep(arr, "merge_end", l, r, "Merge finished");
}
...
int partition(vi& arr, int l, int r) {
    ...
    int i = l - 1;
    for (int j = l; j < r; ++j) {
        logStep(arr, "compare", j, r, "Compare to pivot");
        if (arr[j] <= pivot) {
            swap(arr[++i], arr[j]);
            logStep(arr, "swap", i, j, "Swap");
        }
    }
    swap(arr[i + 1], arr[r]);
...
void insertionSort(vi& arr) {
    ...
    for (int i = 1; i < n; ++i) {
        ...
        while (j >= 0 && arr[j] > key) {
            arr[j + 1] = arr[j];
            logStep(arr, "array-update", -1, -1, "Shift right");
            --j;
        }
        arr[j + 1] = key;
        logStep(arr, "array-update", -1, -1, "Insert key");
    }
}
    ...
    for (int gap = n/2; gap > 0; gap /= 2) {
        for (int i = gap; i < n; ++i) {
            ...
            while (j >= gap && arr[j-gap] > temp) {
                arr[j] = arr[j-gap];
                logStep(arr, "move", j-gap, j, "Shift in gap");
                j -= gap;
            }
            arr[j] = temp;
...
void countingSortByDigit(vi &arr, int exp) {
    ...
    arr = output;
    logStep(arr, "count_sort_pass", -1, -1, "Digit pass exp=" + to_string(exp));
}

void radixSort(vi &arr) {
    int m = *max_element(arr.begin(), arr.end());
    for (int exp = 1; m/exp > 0; exp *= 10) countingSortByDigit(arr, exp);
}
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        cerr << "Error: specify algorithm name\n";
        return 1;
    }
    string algo = argv[1];
    ...
    if (algo == "merge-sort") mergeSort(arr, 0, arr.size()-1);
    else if (algo == "quick-sort") quickSort(arr, 0, arr.size()-1);
    else if (algo == "insertion-sort") insertionSort(arr);
    else if (algo == "selection-sort") selectionSort(arr);
    else if (algo == "bubble-sort") bubbleSort(arr);
    else if (algo == "heap-sort") heapSort(arr);
    else if (algo == "shell-sort") shellSort(arr);
    else if (algo == "radix-sort") radixSort(arr);
    else {
